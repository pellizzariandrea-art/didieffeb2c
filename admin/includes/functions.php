...
// Verifica che tutte le tabelle configurate abbiano record
function verifyTablesHaveRecords($dbConfig) {
    $tableConfig = loadTableConfig();

    // Se non c'è configurazione tabelle, usa la tabella di default dal DB config
    if (!$tableConfig) {
        if (!isset($dbConfig['table'])) {
            throw new Exception("Nessuna configurazione tabella trovata");
        }
    ...
    // Verifica tabella principale
    $mainTable = $tableConfig['mainTable'];
    try {
        $stmt = $pdo->query("SELECT COUNT(*) as count FROM `$mainTable`");
    ...
    // Verifica tabelle in JOIN
    if (isset($tableConfig['joins']) && is_array($tableConfig['joins'])) {
        foreach ($tableConfig['joins'] as $join) {
            $joinTable = $join['table'];
            try {
                $stmt = $pdo->query("SELECT COUNT(*) as count FROM `$joinTable`");
                $count = $stmt->fetch(PDO::FETCH_ASSOC)['count'];
                $results[$joinTable] = [
                    'exists' => true,
                    'count' => $count,
...
function translateText($text, $targetLang, $apiKey, $keepAliveCallback = null) {
    if (empty($text)) {
        return $text;
    }
...
Translation:";
    ...
    // Invia keep-alive prima della chiamata API (che è lenta)
    // Prova prima la callback passata, poi la globale
...
/**
 * Genera products.json con chunked processing e callback per progress real-time
 *
...
function generateProductsJSONChunkedWithCallback($config, $mappings, $translationSettings, $productLimit = null, $filters = [], $imageSettings = [], $variantConfig = [], $ecommerceConfig = [], $progressCallback = null, $chunkSize = 10) {
    ...
    // Callback iniziale
    if ($progressCallback) {
        $progressCallback(0, $totalRows, [
            'phase' => 'loading',
            'message' => "Caricati {$totalRows} prodotti dal database"
        ]);
    }
        ...
        foreach ($chunk as $row) {
            ...
            if ($progressCallback && $processedCount % $chunkSize === 0) {
                ...
                $progressCallback($processedCount, $totalRows, [
                    'phase' => 'transform',
                    'message' => "Trasformati {$processedCount}/{$totalRows} prodotti",
                    'percent' => $percent,
                    'product' => [
    ...
    // FASE 3: Genera metadati e-commerce (30-40%)
    $ecommerceMeta = [];
    if (!empty($ecommerceConfig)) {
        if ($progressCallback) {
            $progressCallback($totalRows, $totalRows, [
                'phase' => 'metadata',
                'message' => 'Generazione metadati e-commerce...',
                'percent' => 35
            ]);
        }
    ...
    // FASE 4: Raggruppamento varianti (40-50%)
    if (!empty($variantConfig['enabled'])) {
        if ($progressCallback) {
            $progressCallback($totalRows, $totalRows, [
                'phase' => 'variants',
                'message' => 'Raggruppamento varianti...',
                'percent' => 40
            ]);
        }
        ...
        // IMPORTANTE: Aggiorna il totale dopo raggruppamento
        $totalRows = count($products);

        if ($progressCallback) {
            $progressCallback($totalRows, $totalRows, [
                'phase' => 'variants',
                'message' => "Raggruppati in {$totalRows} gruppi",
                'percent' => 50
            ]);
        }
    }
    ...
    // FASE 5: Traduzioni (50-90%)
    if (empty($translationSettings['enabled']) || empty($translationSettings['api_key'])) {
        // Nessuna traduzione, salta alla fine
        if ($progressCallback) {
            $progressCallback($totalRows, $totalRows, [
                'phase' => 'saving',
                'message' => 'Finalizzazione export...',
                'percent' => 90
            ]);
        }
    ...
    foreach ($products as $prodIndex => $product) {
        ...
        // Traduci attributi (semplificato per performance)
        if ($translationSettings['translate_attributes'] && !empty($product['attributi'])) {
            $mlAttributes = [];
        ...
        if ($progressCallback && $translatedCount % max(1, floor($totalProducts / 20)) === 0) {
            $progressCallback($translatedCount, $totalProducts, [
                'phase' => 'translation',
                'message' => "Tradotti {$translatedCount}/{$totalProducts} prodotti",
                'percent' => 50 + round(($translatedCount / $totalProducts) * 35)
            ]);
        }
    }

    // Traduci anche i metadati e-commerce
    if (!empty($ecommerceMeta)) {
        if ($progressCallback) {
            $progressCallback($totalProducts, $totalProducts, [
                'phase' => 'metadata',
                'message' => 'Traduzione metadati e-commerce...',
                'percent' => 85
            ]);
        }

        $ecommerceMeta = generateEcommerceMeta($ecommerceConfig, $multilingualProducts, $languages, true);
    }
