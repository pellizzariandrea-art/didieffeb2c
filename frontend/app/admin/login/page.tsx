      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-400">
            {language === 'it' ? 'Area riservata agli amministratori' : language === 'en' ? 'Restricted area for administrators' : language === 'de' ? 'Bereich für Administratoren' : language === 'fr' ? 'Zone réservée aux administrateurs' : language === 'es' ? 'Área reservada para administradores' : 'Área reservada para administradores'}
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm">
              ...
              >
                {language === 'it' ? 'Login cliente' : language === 'en' ? 'Customer login' : language === 'de' ? 'Kunden-Login' : language === 'fr' ? 'Connexion client' : language === 'es' ? 'Acceso de cliente' : 'Login de cliente'}
              </Link>
            </div>
          </div>
        ...
        <div className="mt-6">
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-gray-900 text-gray-400">
                {language === 'it' ? 'Oppure' : language === 'en' ? 'Or' : language === 'de' ? 'Oder' : language === 'fr' ? 'Ou' : language === 'es' ? 'O' : 'Ou'}
              </span>
            </div>
          </div>
          ...
          <div className="mt-6">
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 py-2 px-4 border border-gray-700 rounded-md shadow-sm bg-white hover:bg-gray-50 text-sm font-medium text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              ...
              {language === 'it' ? 'Accedi con Google' : language === 'en' ? 'Sign in with Google' : language === 'de' ? 'Mit Google anmelden' : language === 'fr' ? 'Se connecter avec Google' : language === 'es' ? 'Iniciar sesión con Google' : 'Entrar com Google'}
            </button>
          </div>
        </div>
